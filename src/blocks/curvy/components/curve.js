export const Curve = (props) => {
    ...
    return <div style={{
        position: "absolute", top: !props.isBottom ? 0 : "initial", bottom: props.isBottom ? 0 : "initial", left: "0", width: "100%", overflow: "hidden", height: `${props.height}px`,
        transform: `scaleX(${props.flipX ? -1 : 1}) rotate(${props.flipY ? "180deg" : 0}) scaleY(${props.isBottom ? -1 : 1})`
    }}>
        <svg
            preserveAspectRatio="none"
            style={{ position: "absolute", top: "0", left: "0", height: `${props.height}px`, width: `${props.width}%` }} viewBox="0 0 1200 120">
            <path style={{ fill: props.topColor || "white" }} d={props.flipY ? invertedPath : normalPath}></path>
        </svg>
    </div>;
}